...
let zero_bits count = List.init count (fun _ -> `Zero)
...
let add_termination_bits_if_necessary metadata stream =
  ...
  let termination_bits = if lack_bits >= 4 then zero_bits 4 else zero_bits lack_bits in
...
let make ~segments ~metadata =
  ...
  let stream = add_termination_bits_if_necessary metadata stream in
  ...
  let data = zero_bits reminder_bits in

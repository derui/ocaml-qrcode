...
module Count_indicator = struct
  ...
  let of_mode_with_version ~mode ~version =
    let bits = match mode with
      | Mode.Number -> (match version with
      ...
      | Mode.Alphabet ->(match version with
      ...
      | Mode.Byte -> (match version with
                        | Version.V_1 | V_2 | V_3 | V_4|V_5|V_6|V_7|V_8|V_9 -> 8
                        | _ ->16
                       )
  in
  {bits; count = 0}
